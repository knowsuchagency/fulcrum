...
export function CreateTaskModal({ open: controlledOpen, onOpenChange }: CreateTaskModalProps = {}) {
  ...
  const [aiMode, setAiMode] = useState<'default' | 'plan' | 'none'>('none')
  ...
  // Auto-switch to 'plan' mode when user starts typing a description
  useEffect(() => {
    if (description.trim() && aiMode === 'none') {
      setAiMode('plan')
    }
  }, [description, aiMode])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createTask.mutate(
      ...
      {
        onSuccess: (task) => {
          const navState = aiMode !== 'none' && description.trim()
              ...
              <Field>
                ...
                <ToggleGroup
                  ...
                  onValueChange={(value) => {
                    ...
                    if (selected) setAiMode(selected as 'default' | 'plan' | 'none')
                  }}
                  className="w-full"
                  variant="outline"
                >
