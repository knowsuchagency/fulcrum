#!/bin/bash
# Fulcrum Desktop Launcher for Linux
...
SELF=$(readlink -f "$0")
APP_DIR="${SELF%/*}"
BUNDLE_DIR="$APP_DIR/../share/fulcrum/bundle"
...
error_dialog() {
  ...
  if command -v zenity &> /dev/null; then
    zenity --error --title="Fulcrum" --text="$1" 2>/dev/null || echo "ERROR: $1" >&2
  elif command -v kdialog &> /dev/null; then
    kdialog --error "$1" --title "Fulcrum" 2>/dev/null || echo "ERROR: $1" >&2
  else
    echo "ERROR: $1" >&2
  fi
}
...
should_start_server() {
  ...
  REMOTE_HOST=$(jq -r '.remoteFulcrum.host // .remoteHost // ""' "$SETTINGS_FILE" 2>/dev/null)
