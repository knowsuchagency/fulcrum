...
LOG_FILE="$FULCRUM_DIR/desktop.log"
PID_FILE="$FULCRUM_DIR/desktop-server.pid"
PORT="${PORT:-7777}"
...
error_dialog() {
  osascript -e "display dialog \"$1\" buttons {\"OK\"} default button \"OK\" with title \"Fulcrum\" with icon stop" 2>/dev/null || echo "$1" >&2
}

info_dialog() {
  osascript -e "display dialog \"$1\" buttons {\"OK\"} default button \"OK\" with title \"Fulcrum\"" 2>/dev/null || echo "$1" >&2
}
...
check_claude() {
  if ! command -v claude &> /dev/null; then
    ...
    export FULCRUM_CLAUDE_MISSING=1
  else
    log "Claude Code CLI found: $(which claude)"
    export FULCRUM_CLAUDE_MISSING=0
  fi
}
...
install_plugin() {
  ...
  PLUGIN_DEST="$HOME/.claude/plugins/fulcrum"
...
# Start the server
start_server() {
  SERVER_BIN="$BUNDLE_DIR/server/fulcrum-server"
  ...
  NODE_ENV=production \
  PORT="$PORT" \
  FULCRUM_DIR="$FULCRUM_DIR" \
  FULCRUM_PACKAGE_ROOT="$BUNDLE_DIR" \
  BUN_PTY_LIB="$PTY_LIB" \
  FULCRUM_CLAUDE_MISSING="${FULCRUM_CLAUDE_MISSING:-0}" \
  ...
  error_dialog "Failed to start Fulcrum server. Check $LOG_FILE for details."
  exit 1
}
...
# Main
log "=== Fulcrum Desktop starting ==="
log "APP_DIR: $APP_DIR"
log "BUNDLE_DIR: $BUNDLE_DIR"
log "FULCRUM_DIR: $FULCRUM_DIR"
