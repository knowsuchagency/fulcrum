...
- **并行代理编排** — 跨不同任务和工作树运行多个 AI 代理会话。在一个并行视图中查看和控制所有会话。
- **随时随地工作** — 合上笔记本电脑——您的代理继续工作。用手机随时接续之前的进度。
- **多代理支持** — 选择 Claude Code 或 OpenCode。支持全局默认、按仓库覆盖或按任务选择。
...
从计划到完成，跟踪任务进度。创建任务时自动生成隔离的 git 工作树，并在您使用 AI 代理工作时实时更新状态。

![看板](/screenshots/tasks-kanban-board.png)

### 任务终端视图

在一个并行视图中查看所有任务和工作树的 AI 代理会话。每个任务运行在独立的 git 工作树中，您可以同时监控和交互所有会话。
...
在分屏视图中预览应用和代理终端。当 AI 代理迭代您的代码时，实时观察变化。
