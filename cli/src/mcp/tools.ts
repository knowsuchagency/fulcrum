...
const ToolCategorySchema = z.enum(['core', 'tasks', 'projects', 'repositories', 'apps', 'filesystem', 'git', 'notifications', 'exec', 'settings', 'backup', 'email'])
...
export function registerTools(server: McpServer, client: FulcrumClient) {
  ...
  server.tool(
    ...
    'Create a new task. For worktree tasks, provide repoPath to create a git worktree. For non-worktree tasks, omit repoPath. When tags are provided, returns all existing tags for reference.',
    {
      ...
      status: z.optional(TaskStatusSchema).describe('Initial status (default: TO_DO, use IN_PROGRESS for immediate worktree creation)'),
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        return formatSuccess({
          settings,
          categories: ['server', 'paths', 'editor', 'integrations', 'agent', 'tasks', 'appearance', 'assistant'],
  ...
  server.tool(
    ...
    {
      key: z.string().describe('Setting key in dot notation (e.g., "appearance.theme", "server.port")'),
    },
    async ({ key }) => {
      try {
  ...
  server.tool(
    ...
    {
      key: z.string().describe('Setting key in dot notation (e.g., "appearance.theme", "server.port")'),
      value: z.union([z.string(), z.number(), z.boolean(), z.null()]).describe('New value for the setting'),
    },
    async ({ key, value }) => {
      try {
        // Validate sensitive settings aren't being read back
  ...
  server.tool(
    ...
    {
      name: z.string().describe('Backup name (timestamp format like 2024-01-15T10-30-00-000Z)'),
    },
    async ({ name }) => {
      try {
  ...
  server.tool(
    ...
    'Search emails via IMAP. Results are fetched from the mail server and stored locally. Use this for searching emails beyond what is already stored.',
    {
      ...
      fetchLimit: z.optional(z.number()).describe('Maximum emails to fetch and store (default: 20, set to 0 to only get UIDs)'),
    },
    async ({ subject, from, to, since, before, text, seen, flagged, fetchLimit }) => {
      try {
        ...
        return formatSuccess({
          ...result,
          hint: result.matchCount > result.fetched
