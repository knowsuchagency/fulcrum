...
import { shouldDropTerminalData } from '@/lib/terminal-data-filter'
    ...
    return {
      ...
      attachXterm(
        ...
        const disposable = xterm.onData((data) => {
          // Drop xterm.js-generated query responses (mouse motion, OSC/DCS
          // responses, DECRPM/DECRQSS mode reports, cursor-position and
          // device-attributes reports) that would otherwise be forwarded to the
          // PTY as if the user had typed them. See shouldDropTerminalData for why
          // every rule must stay anchored: a paste arrives as a single onData
          // event, so an unanchored rule would silently drop pasted text that
          // merely contains a response-like substring, e.g. `echo "$year"`.
          if (shouldDropTerminalData(data)) {
            return
          }
          getWs().sendBinary(
            encodeOpcodeFrame(OPCODE_INPUT, terminalId, encodeStringPayload(data))
