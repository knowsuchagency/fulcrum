...
function JobsTab({ scopeFilter, onScopeChange }: { scopeFilter: 'all' | 'user' | 'system'; onScopeChange: (scope: 'all' | 'user' | 'system') => void }) {
  const { t } = useTranslation('jobs')
...
function MonitoringPage() {
  ...
  const { tab: tabFromUrl, scope: scopeFromUrl } = Route.useSearch()

  const activeTab = tabFromUrl || 'system'
  const jobScope = scopeFromUrl || 'all'
  ...
  const setJobScope = useCallback((scope: JobScope) => {
    navigate({
      to: '/monitoring',
      search: scope === 'all' ? { tab: 'jobs' } : { tab: 'jobs', scope },
      replace: true,
    })
  }, [navigate])

  const tabs: { value: MonitoringTab; label: string; devOnly?: boolean; jobsOnly?: boolean }[] = [
    ...
    { value: 'jobs', label: tj('title'), jobsOnly: true },
    { value: 'usage', label: t('tabs.usage') },
  ]

  const visibleTabs = tabs.filter(tab => {
    if (tab.devOnly && !developerMode?.enabled) return false
    if (tab.jobsOnly && !jobsAvailable?.available) return false
    return true
  })
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        {activeTab === 'jobs' && jobsAvailable?.available && <JobsTab scopeFilter={jobScope} onScopeChange={setJobScope} />}
