...
export function TaskDependencyGraph({ className, projectFilter, tagsFilter, taskTypesFilter, prioritiesFilter }: TaskDependencyGraphProps) {
  ...
  const { initialNodes, initialEdges } = useMemo((): { initialNodes: Node<TaskNodeData>[]; initialEdges: Edge[] } => {
    ...
    // Helper to check if a task matches the task type filter
    const taskMatchesTypeFilter = (task: TaskGraphNode): boolean => {
      if (!taskTypesFilter || taskTypesFilter.length === 0) return true
      const fullTask = taskMap.get(task.id)
